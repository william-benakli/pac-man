...
int gameInput(int socketclient, struct participant *partcipant_ingame, struct game *game_courant){
  ...
  while(check_endgame(game_courant) == NOT_FINISH){
    ...
    if(count_fst == 0){
      printf("recu 0 bits de read ligne 12 donc client deconnecté");
      return 0;
    }
    ...
    if (strcmp(buffer,"MALL?") == 0){
      ...
      if (test_mall != 0){
        ...
        int bytes_sent = send(socketclient,"NMALL***",9 * sizeof(char),0);
        continue;
      }
    ...
    if (strcmp(buffer,"SEND?") == 0){
      ...
      if (test_private != 0){
        ...
        int bytes_sent = send(socketclient,"NSEND***",9 * sizeof(char),0);
        continue;
      }
