...
int select_action(struct player * player, char *buffer){
    ...
    if(strcmp(buffer, CMD_NEW_PARTY) == 0){
        int rep_create = creategame(player, _games); 
            if(rep_create == -1) sendRegNo(socketclient);
            else sendRegOk(socketclient, player->game_id);
    }else if(strcmp(buffer, CMD_REGISTER) == 0){
        int rep_regis = regisgame(player, _games); 
            if(rep_regis == -1)sendRegNo(socketclient);
            else sendRegOk(socketclient, player->game_id); 
    }else if(strcmp(buffer, CMD_UNREG) == 0){

        int check_stars = readStars(socketclient);
        if(check_stars == -1)sendDunno(socketclient, "CMD UNREG READSTARS ERROR");

        if(player->status_game == IN_LOBBY)sendDunno(socketclient, "UNREG ERREUR PLAYER EN LOBBY");
      
        u_int8_t id_partie = player->game_id;
        int rep_unreg = unregis(player, _games);
        if(rep_unreg == -1)sendDunno(socketclient, "UNREG ERREUR UNREG PLAYER LIST");

        int rep_regOk = sendUnRegOk(socketclient, id_partie);
        if(rep_regOk == -1)sendDunno(socketclient, "UNREG ERREUR UNREGOK FAIL");

    }else if(strcmp(buffer, CMD_GAME) == 0){
        int check_stars = readStars(socketclient);
        if(check_stars == -1)sendDunno(socketclient, "CMD GAME READSTART ERROR");
        int rep_party = sendgames(socketclient);
        if(rep_party == -1)sendDunno(socketclient, "CMD GAME ENVOIE GAME ECHEC");
    }else if(strcmp(buffer, CMD_SIZE) == 0){
        int rep_size = sendSize(socketclient, _games);
        if(rep_size == -1)sendDunno(socketclient, "CMD SIZE sendSIZE FAIL");
    }else if(strcmp(buffer, CMD_LIST) == 0){
        int rep_list = sendList(socketclient, _games);
        if(rep_list == -1)sendDunno(socketclient,"CMD LIST sendList fail");
    }else if(strcmp(buffer, CMD_START) == 0){
        int rep_list = player_game_accept(player, _games);
        if(rep_list == -1) sendDunno(socketclient,"CMD START FAILED");
        int rep_stars = readStars(socketclient);
        if(rep_stars == -1)sendDunno(socketclient,"CMD START FAILED miss ***");
        int rep_waiting = waiting_players(player);
        if(rep_waiting)sendDunno(socketclient, "CMD WAITING OR LAUNCH FAILED");
    }else{
