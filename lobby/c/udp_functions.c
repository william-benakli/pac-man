...
int broadcast_message(struct game *_game, char *buf){
    ...
    if(r==0){
        ...
        sendto(sock, buf, strlen(buf), 0, saddr,(socklen_t)sizeof(struct sockaddr_in));
        }
    }
    return 0;
}
...
int score_message(struct game *_game, struct participant *player, int fantom_x , int fantom_y){
    ...
    int bytes_written = sprintf(score_buffer,"SCORE %s %04d %03d %03d+++",player->identifiant,player->score,fantom_x,fantom_y);
    if (bytes_written < 0){
        return -1;
    }
...
int ghost_message(struct game *_game, int fantom_x , int fantom_y){
    ...
    int bytes_written = sprintf(ghost_buffer,"GHOST %03d %03d+++",fantom_x,fantom_y);
    if (bytes_written < 0){
        return -1;
    }
...
int end_message(struct game *_game, struct participant *winner){
    char end_buffer[30];
    int bytes_written = sprintf(end_buffer,"ENDGA %s %04d+++",winner->identifiant,winner->score);
    if (bytes_written < 0){
        return -1;
    }
...
int group_message(struct game *_game, struct participant *sender, char *message){
    ...
    int bytes_written = sprintf(message_buffer,"MESSA %s %s+++",sender->identifiant,message);
    if (bytes_written < 0){
        return -1;
    }
...
int private_message(struct game *_game, char *target_identifiant, char *message, struct participant *sender){
    ...
    int bytes_written = sprintf(private_buffer,"MESSP %s %s+++",sender->identifiant,message);
    if (bytes_written < 0){
        return -1;
    }
    ...
    if(r==0){
        if(first_info!=NULL){
        struct sockaddr *saddr=first_info->ai_addr;
        sendto(sock, private_buffer, strlen(private_buffer), 0, saddr,(socklen_t)sizeof(struct sockaddr_in));
        }
    }
    return 0;
...
int get_mall_message(int clientsocket, char message_buffer[200]){
...
int get_private_message(int clientsocket, char target_identifiant[8], char message_buffer[200]){
